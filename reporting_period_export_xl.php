...
$filename ="reporting_period_report.csv";
...
header('Content-Disposition: attachment; filename=reporting_period_report'.date("Y-m-d").'.csv;');
  
    $f = fopen('php://output', 'w');
    $fields = array('ID','reporting period','Comp Code','Added Date');
    fputcsv($f, $fields);
    ...
    while($row = mysql_fetch_assoc($query)){
        $lineData = array($row['id'],$row['reporting_period'],$row['comp_code'],$row['added_date']);
        fputcsv($f, $lineData);
    }
    fseek($f, 0);
    fpassthru($f);
