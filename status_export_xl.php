...
header('Content-Disposition: attachment; filename= 	status_report'.date("Y-m-d").'.csv;');
  
    $f = fopen('php://output', 'w');
    $fields = array('ID','status','Comp Code','Added Date');
    fputcsv($f, $fields);
    ...
    while($row = mysql_fetch_assoc($query)){
        ...
        $name12=getrecord("tbl_status1","name","where id='".$row['status']."'");
