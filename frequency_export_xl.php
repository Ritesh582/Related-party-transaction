...
if(isset($_SESSION['branch_code_tmp']) && !empty($_SESSION['branch_code_tmp'])){
 ...
 header('Content-Type: text/csv');
  header('Content-Disposition: attachment; filename=frequency_report'.date("Y-m-d").'.csv;');
  
    $f = fopen('php://output', 'w');
    $fields = array('ID','frequency','Comp Code','Added Date');
    fputcsv($f, $fields);
    ...
    while($row = mysql_fetch_assoc($query)){
        $name12=getrecord("tbl_frequency","name","where id='".$row['frequency']."'");
