...
$dropdown_Arr = array(
    ...
    'match_status' => array ("m"=> "Match", "u"=>"UnMatch"),
);
$userType = array(1=>'HEAD',2=>'Others') ;
$admin_user =array(1=>'Technicals',2=>'Accountants',3=>'Admin');
$emailstatus=array(0=>"Pending",1=>"Success",2=>"Failed",3=>"Invalid Email Id",4=>"From, CC and BCC Emails are not Defined",5=>"Data Not Found",6=>"Template Not Found",7=>"More then 100 tax difference");
...
function dropdownuploadbank($tablename,$sid="",$cnd=""){
    global $Db;
    $q = "select id,name,bank_code from ".$tablename;
    $q .= $cnd;
    $result =	$Db->query($q);
    $str = "";
    while($ft = mysql_fetch_object($result)){
            $str .="<option value='".$ft->bank_code."' ".strselected($ft->id,$sid).">".wordWrapper($ft->name,150)."</option>";
    }
    return $str;
}
function dropdownbank($tablename,$sid="",$cnd="",$name){
    global $Db;
    $q = "select id,name from ".$tablename;
    $q .= $cnd;
    $result =	$Db->query($q);
    ...
    while($ft = mysql_fetch_object($result)){
         if($name==$ft->name){
              $str .="<option value='".$ft->name."' ".strselected($ft->id,$sid)." selected>".wordWrapper($ft->name,150)."</option>";
         }else{
            $str .="<option value='".$ft->name."' ".strselected($ft->id,$sid).">".wordWrapper($ft->name,150)."</option>";
         }
    }
    return $str;
}
function dropdowncomp($tablename,$sid="",$cnd,$name){
    global $Db;
    $q = "select id,compname from ".$tablename;
    $q .= $cnd;
    $result =	$Db->query($q);
    $str = "";
    while($ft = mysql_fetch_object($result)){
        if($name==$ft->compname)
        {
            $str .="<option value='".$ft->compname."' ".strselected($ft->id,$sid)." selected>".wordWrapper($ft->compname,150)."</option>";
        }else{
            $str .="<option value='".$ft->compname."' ".strselected($ft->id,$sid).">".wordWrapper($ft->compname,150)."</option>";
        }
...
function dropdown_name($q, $selected=''){
    global $Db;
    $selected = explode(',',$selected);
    $str = "";
    $q = $Db->query($q);
    while($ft = mysql_fetch_object($q)){
        if(in_array($ft->name,$selected)){
                $str .= "<option value='".$ft->name."' selected>".$ft->name."</option>";
        }else{
                $str .= "<option value='".$ft->name."'>".$ft->name."</option>";
        }
    }
    return $str;
}
