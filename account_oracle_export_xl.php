...
$filename = "account_oracle_report.csv";
...
header('Content-Disposition: attachment; filename=account_oracle_report'.date("Y-m-d").'.csv;');
  
    $f = fopen('php://output', 'w');
    $fields = array('ID','account oracle','Comp Code','Added Date');
    fputcsv($f, $fields);
    ...
    while($row = mysql_fetch_assoc($query)){
        $lineData = array($row['id'],$row['account_oracle'],$row['comp_code'],$row['added_date']);
        fputcsv($f, $lineData);
    }
    fseek($f, 0);
    fpassthru($f);
