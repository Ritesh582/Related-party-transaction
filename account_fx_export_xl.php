...
$filename = "account_fx_report.csv";
...
header('Content-Disposition: attachment; filename=account_fx_report'.date("Y-m-d").'.csv;');
  
    $f = fopen('php://output', 'w');
    $fields = array('ID','account fx','Comp Code','Added Date');
    fputcsv($f, $fields);
    ...
    while($row = mysql_fetch_assoc($query)){
        $lineData = array($row['id'],$row['account_fx'],$row['comp_code'],$row['added_date']);
        fputcsv($f, $lineData);
    }
    fseek($f, 0);
    fpassthru($f);
