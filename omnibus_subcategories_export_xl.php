...
$filename = "omnibus_subcategories_report.csv";
...
header('Content-Disposition: attachment; filename= omnibus_subcategories_report'.date("Y-m-d").'.csv;');
  
    $f = fopen('php://output', 'w');
    $fields = array('ID','omnibus subCategories','Comp Code','Added Date');
    fputcsv($f, $fields);
    ...
    while($row = mysql_fetch_assoc($query)){
        $lineData = array($row['id'],$row['sub_category'],$row['comp_code'],$row['added_date']);
        fputcsv($f, $lineData);
    }
    fseek($f, 0);
    fpassthru($f);
