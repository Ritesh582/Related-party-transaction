...
class styleHTML
{
	protected	$pdf			= null;			// référence au PDF parent
	protected	$htmlColor		= array();		// liste des couleurs HTML
	protected	$onlyLeft		= false;		// indique si on est dans un sous HTML et qu'on bloque à gauche
	protected	$defaultFont	= null;			// fonte par défaut si la fonte demandée n'existe pas
	
	public		$value			= array();		// valeurs actuelles
	public		$css			= array();		// tableau des CSS
	public		$css_keys		= array();		// tableau des clefs CSS, pour l'ordre d'execution
	public		$table			= array();		// tableau d'empilement pour historisation des niveaux
	...
	*/
	 ...
	 * @return	null
	 */
	public function resetStyle($balise = '')
	{
	...
	public function FontSet()
	{
		...
		$this->pdf->SetFont($family, $b.$i.$u.$d.$o, $this->value['mini-size']*$size);
	...
	public function getSvgStyle($balise, &$param)
	{
		// preparation
	 ...
	 * @return	null
	 */
	public function analyse($balise, &$param, $heritage = null)
	{
		// preparation
		$balise = strtolower($balise);
		$id		= isset($param['id'])		? strtolower(trim($param['id']))	: null; if (!$id)	$id		= null;
